  ...
  template: `
    ...
    >
      ...
      <div class="ld__stage">
        ...
        <h1 class="ld__word" [class.ld__word--out]="wordOut()" aria-label="Maya">
          <svg
            viewBox="0 0 246.75 91.68"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
          >
  ...
  styles: [
    `
      ...
      .ld {
        ...
        background:
          radial-gradient(1100px 520px at 50% 14%, rgba(220, 88, 42, 0.2), transparent 70%), #0c0a09;
        color: #f6f3f0;
        font-family: var(--font-display, 'Plus Jakarta Sans', system-ui, sans-serif);
        overflow: hidden;
        /* Salida: cortina que se recorta de abajo hacia arriba (timing OUTFIT:
           1.4s power2.inOut ≈ cubic-bezier(0.455, 0.03, 0.515, 0.955)) */
        clip-path: inset(0% 0% 0% 0%);
        contain: layout paint style;
        transition: clip-path 1.4s cubic-bezier(0.455, 0.03, 0.515, 0.955);
      }
      .ld--out {
      ...
      .ld__card {
        ...
        /* Colapso OUTFIT: 0.6s expo.inOut ≈ cubic-bezier(0.87, 0, 0.13, 1) */
        transition:
      ...
      /* Aparición letra por letra — timing OUTFIT: sube desde 110%, 0.6s
         power3.out ≈ cubic-bezier(0.215, 0.61, 0.355, 1), stagger 0.2s en
         orden pseudoaleatorio (--d por letra), tras un delay global de 0.4s. */
      .ld__word .ld__ltr {
        transform-box: fill-box;
        transform-origin: center bottom;
        opacity: 0;
        animation: ld-letter-in 0.6s cubic-bezier(0.215, 0.61, 0.355, 1) both;
        animation-delay: calc(0.4s + var(--d, 0) * 1s);
      }
      ...
      /* Salida OUTFIT: hacia arriba -120%, 1.2s expo.inOut ≈
         cubic-bezier(0.87, 0, 0.13, 1), stagger 0.08s pseudoaleatorio. */
      .ld__word--out .ld__ltr {
        animation: ld-letter-out 1.2s cubic-bezier(0.87, 0, 0.13, 1) both;
      ...
      /* Salida del contador — OUTFIT: sube -100% y se funde, 1s circ.inOut ≈
         cubic-bezier(0.785, 0.135, 0.15, 0.86). Sigue contando mientras sale. */
      .ld__count--out {
        animation: ld-count-out 1s cubic-bezier(0.785, 0.135, 0.15, 0.86) forwards;
      }
...
export class Loader implements OnInit, OnDestroy {
  ...
  /** Duración de la rampa del contador 000→100 (timing OUTFIT: 3s circ.inOut). */
  readonly minMs = input(3000);
  ...
  /** Las imágenes colapsan a scale(0) (primer paso de la salida). */
  protected readonly collapsing = signal(false);
  /** El wordmark sale letra por letra (segundo paso, tras el colapso). */
  protected readonly wordOut = signal(false);
  ...
  ngOnInit(): void {
    ...
    // Rampa OUTFIT: el contador va de 000 a 100 en minMs con ease circ.inOut
    // (arranca lento, acelera, frena). Si los assets tardan más que la rampa,
    // el contador espera en 99 hasta que estén (o hasta maxMs de seguridad).
    // setInterval (no rAF) para que avance aunque la pestaña esté en fondo.
    ...
    this.timer = setInterval(() => {
      ...
      const listo = loadFrac >= 1 || elapsed >= this.maxMs();
      const eased = circInOut(Math.min(1, elapsed / this.minMs())) * 100;
      this.progressRaw = listo ? eased : Math.min(eased, 99);
      this.progress.set(Math.round(this.progressRaw));

      // OUTFIT dispara la salida al 90% del conteo — el contador termina de
      // llegar a 100 mientras ya está saliendo (solapamiento).
      if (listo && elapsed >= this.minMs() * 0.9) this.startExit();
    }, 40);
  }
  ...
  /** Stagger del colapso — OUTFIT: 0.1s "desde el final" (la del frente sale última). */
  protected cardDelay(i: number): string {
    if (!this.collapsing()) return '0s';
    const d = Math.max(0, this.top() - i);
    return `${(3 - Math.min(d, 3)) * 0.1}s`;
  }

  private exiting = false;

  /** Secuencia de salida con los timings de OUTFIT (todo se solapa):
   *  t=0     → contador sube y se funde (1s) · letras salen (1.2s) · cartas colapsan (0.6s)
   *  t=360   → 30% de la salida de letras: arranca la cortina clip-path (1.4s)
   *  t=1060  → 50% de la cortina: clase `loaded` + evento — el home EMPIEZA a
   *            entrar mientras la cortina aún sube (ese solape es el truco)
   *  t=1760  → fin de la cortina: se desmonta el loader
   */
  private startExit(): void {
    if (this.exiting) return;
    this.exiting = true;
    this.collapsing.set(true);
    this.wordOut.set(true);
    setTimeout(() => this.leaving.set(true), 360);
    setTimeout(() => {
      this.doc.documentElement.classList.add('loaded');
      this.doc.defaultView?.dispatchEvent(new CustomEvent('maya-loaded'));
    }, 1060);
    setTimeout(() => {
