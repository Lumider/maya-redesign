...
@Component({
  ...
  template: `
    ...
    >
      ...
      <div class="ld__stage">
        ...
        <h1 class="ld__word" [class.ld__word--out]="collapsing()" aria-label="Maya">
  ...
  styles: [
    `
      ...
      /* Salida del wordmark: cada letra se va (fade + cae + encoge), escalonada */
      .ld__word--out .ld__ltr {
        animation: ld-letter-out 0.4s cubic-bezier(0.4, 0, 1, 1) forwards;
        animation-delay: calc(var(--i, 0) * 0.07s);
      }
      @keyframes ld-letter-out {
        from { opacity: 1; transform: translateY(0) scale(1); }
        to { opacity: 0; transform: translateY(22%) scale(0.6); }
      }
