...
@Component({
    ...
    <div class="v2">
      <header class="v2-head" appReveal>
        ...
        <div class="grupo-toggle" role="tablist" aria-label="Grupos de Mi equipo">
          <button role="tab" [attr.aria-selected]="grupo()==='gp'" [class.on]="grupo()==='gp'" (click)="grupo.set('gp')">
            <app-icon name="users" [size]="16" /> Mi Grupo Personal
          </button>
          <button role="tab" [attr.aria-selected]="grupo()==='dir'" [class.on]="grupo()==='dir'" (click)="grupo.set('dir')">
      ...
      @if (grupo() === 'gp') {
        <div class="gp-ind" appReveal>
          <div class="tile card"><div class="tile__top"><span class="tile__label">Activas</span><span class="sem sem--ok"></span></div><div class="tile__value">{{ gpActivas }}<span class="muted" style="font-size:15px">/{{ gpMeta }}</span></div><div class="tile__hint">activas del GP esta campaña</div></div>
          <div class="tile card"><div class="tile__top"><span class="tile__label">En riesgo</span><span class="sem sem--warn"></span></div><div class="tile__value">{{ gpEnRiesgo }}</div><div class="tile__hint">por reactivar o con deuda</div></div>
          <div class="tile card"><div class="tile__top"><span class="tile__label">Créditos</span><span class="sem" [class]="pendientes().length ? 'sem--bad' : 'sem--ok'"></span></div><div class="tile__value">{{ pendientes().length }}</div><div class="tile__hint">por aprobar</div></div>
        </div>
          ...
          <main>
            @if (pendientes().length) {
              <section class="card pad cred v2-section" appReveal>
                ...
                <p class="tiny" style="margin:-4px 0 12px">Es crédito a <strong>tus</strong> consultoras: sin tu aprobación no pueden pasar pedido, y aprobar suma a tu deuda GP (IM).</p>
            ...
            <section id="consultoras" class="v2-section" appReveal>
              ...
              <input class="search" type="search" [value]="q()" (input)="q.set($any($event.target).value)" placeholder="Buscar consultora…" aria-label="Buscar consultora" />
              <div class="chips">
                ...
                @for (cn of visibles(); track cn.nombre) {
                  <div class="cn">
                    ...
                    <div class="cn__info">
                      ...
                      <span class="tiny">{{ cn.nivel }} · personal \${{ cn.ventaPersonal | number }}@if (cn.ventaGrupal) { · grupal \${{ cn.ventaGrupal | number }} }</span>
                    </div>
                    <div class="cn__tags">
                      @if (cn.estado==='deuda') { <span class="badge badge--danger">Deuda</span> }
                      @if (cn.estado==='nueva') { <span class="badge badge--violet">Nueva</span> }
                      @if (tieneCredito(cn)) { <span class="badge badge--info">Crédito pend.</span> }
                    </div>
      ...
      @if (grupo() === 'dir') {
        ...
        <section class="card pad v2-section" appReveal>
          <div class="resumen">
            <div class="resumen__ring">
              ...
              <span class="tiny" [class.bad]="pctHijasA < 60">{{ pctHijasA }}% · meta 60% (Medalla de Liderazgo)</span>
            </div>
            <div class="resumen__dist">
              <span class="tiny" style="font-weight:700">Distribución por cuadrante · {{ directoras.length }} directoras</span>
              <div class="dist-bar" role="img" [attr.aria-label]="'A ' + dist.A + ', B ' + dist.B + ', C ' + dist.C + ', D ' + dist.D">
                @for (q of dist.partes; track q.k) {
                  @if (q.n) { <span class="dist-bar__seg" [class]="'dist-bar__seg--' + q.k.toLowerCase()" [style.flex]="q.n">{{ q.k }} {{ q.n }}</span> }
                }
              </div>
              <div class="gen-counts">
                <span><strong>{{ gen[1] }}</strong> hijas</span>
                <span><strong>{{ gen[2] }}</strong> nietas</span>
                <span><strong>{{ gen[3] }}</strong> bisnietas</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <div class="lentes chips" appReveal role="tablist" aria-label="Lente de la lista">
          @for (l of lentes; track l.id) {
            <button class="chip" role="tab" [attr.aria-selected]="lente()===l.id" [class.chip--active]="lente()===l.id" (click)="lente.set(l.id)">{{ l.label }}</button>
          }
        </div>
        ...
        <div class="card lista dir-lista v2-section" appReveal>
          @for (d of principales(); track d.nombre) {
            <div class="dir-row" [class.dir-row--open]="abierta()===d.nombre">
              <button class="dir-row__head" [attr.aria-expanded]="abierta()===d.nombre" (click)="toggle(d.nombre)">
                ...
                <span class="dir-row__info">
                  <strong>{{ d.nombre }} <span class="gen">{{ genLabel(d.generacion) }}</span></strong>
                ...
                <div class="dir-profile">
                  <div class="dim-grid">
                    <div class="dim">
                      ...
                      <div class="dim__row"><span class="qbadge" [class]="'qbadge--' + d.cuadrante.toLowerCase()">{{ d.cuadrante }}</span><span class="tiny">{{ d.cuadrante==='A' ? 'En Crecimiento — cumple MRM + PPED' : d.faltaA }}</span></div>
                    </div>

                    <div class="dim">
                      <span class="dim__h"><app-icon name="target" [size]="14" /> Metas · proyección vs avance</span>
                      @for (m of metasArr(d); track m.label) {
                        <div class="mp">
                          <span class="mp__l">{{ m.label }}</span>
                          <div class="progress" [class.progress--success]="m.real >= m.meta"><div class="progress__fill" [style.width.%]="ratio(m.real, m.meta)"></div></div>
                          <span class="mp__v">{{ m.dinero ? '$' : '' }}{{ m.real | number }}<span class="muted">/{{ m.dinero ? '$' : '' }}{{ m.meta | number }}</span></span>
                        </div>
                      }
                    </div>

                    <div class="dim">
                      <span class="dim__h"><app-icon name="users" [size]="14" /> Emprendedoras</span>
                      <div class="dim__row"><span class="empr-n"><strong>{{ d.emprendedoras.cem }}</strong> CEM · <strong>{{ d.emprendedoras.ces }}</strong> CES</span></div>
                      <span class="tiny">{{ d.emprendedoras.nota }} · crecimiento por venta/IM/PPED/activas (últ. 4 campañas)</span>
                      @if (d.emprendedoras.ces===0) { <span class="badge badge--warning" style="margin-top:6px">Falta formar CES</span> }
                    </div>

                    <div class="dim">
                      <span class="dim__h"><app-icon name="star" [size]="14" /> PAR+</span>
                      <div class="dim__row par-row"><span class="par-est">{{ d.par.proyectada ? '★'.repeat(d.par.proyectada) : '—' }}</span><span class="tiny">Proyecta {{ d.par.proyectada ? 'Estrella ' + d.par.proyectada : 'sin proyección' }} · Sueño Estrella {{ d.par.sueno }}</span></div>
                      <span class="tiny">{{ d.par.falta }}</span>
                    </div>
                  </div>
                  <div class="dim__act">
                    <button class="btn btn--primary btn--sm"><app-icon name="users" [size]="15" /> Agendar coaching</button>
          ...
          @if (sanas().length) {
            <div class="sanas">
              <span class="tiny" style="font-weight:700">En Cuadrante A — sanas ({{ sanas().length }})</span>
              <div class="sanas__chips">
                @for (d of sanas(); track d.nombre) {
                  <button class="sana-chip" (click)="grupo.set('dir'); abrirEn(d.nombre)">
                    <span class="sem sem--ok"></span>{{ d.nombre }} <span class="gen">{{ genLabel(d.generacion) }}</span>
                  </button>
                }
              </div>
            </div>
        ...
        <p class="tiny dir-foot" appReveal>
          Cada directora es <strong>empresaria independiente</strong> y gestiona el crédito de su propio grupo: por eso aquí no apruebas créditos.
          Su Cuadrante y PAR+ son de ella — los tuyos viven en <a routerLink="/n/negocio">Mi negocio</a> y <a routerLink="/n/carrera">Mi carrera</a>.
        </p>
      }
    </div>
  ...
  styles: [
    `
      ...
      .grupo-toggle { display: inline-flex; gap: 4px; margin-top: 14px; padding: 4px; border-radius: 99px; background: var(--sand); border: 1px solid var(--line); }
      .grupo-toggle button { display: inline-flex; align-items: center; gap: 7px; border: 0; background: transparent; color: var(--ink-2); font-weight: 700; font-size: 13.5px; padding: 9px 18px; border-radius: 99px; transition: all 0.15s ease; }
      .grupo-toggle button.on { background: var(--ink); color: var(--on-ink); }

      /* Indicadores GP */
      .gp-ind { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 22px; }
      ...
      .search { width: 100%; margin-bottom: 12px; padding: 11px 16px; border-radius: var(--radius-s); border: 1px solid var(--line-strong); background: var(--surface); color: var(--ink); font-size: 14px; font-family: inherit; }
      ...
      .cn__info { flex: 1; display: flex; flex-direction: column; gap: 1px; min-width: 0; }
      .cn__tags { display: flex; gap: 6px; flex-wrap: wrap; }
      ...
      .resumen { display: flex; gap: 26px; align-items: center; flex-wrap: wrap; }
      .resumen__ring { display: flex; flex-direction: column; align-items: center; gap: 6px; text-align: center; }
      .resumen__dist { flex: 1; min-width: 240px; display: flex; flex-direction: column; gap: 10px; }
      .dist-bar { display: flex; gap: 4px; height: 30px; }
      .dist-bar__seg { display: grid; place-items: center; border-radius: 6px; font-size: 11px; font-weight: 800; color: #fff; min-width: 38px; }
      .dist-bar__seg--a { background: var(--fill-success); }
      .dist-bar__seg--b { background: var(--fill-brand); }
      .dist-bar__seg--c { background: #8a5a13; }
      .dist-bar__seg--d { background: var(--fill-danger); }
      .gen-counts { display: flex; gap: 18px; }
      .gen-counts span { font-size: 12.5px; color: var(--ink-2); }
      .gen-counts strong { font-family: var(--font-display); font-size: 18px; color: var(--ink); margin-right: 3px; }

      .lentes { margin-bottom: 4px; }
      .lente-hint { margin: 0 0 14px; }

      /* Lista única de directoras (filas expandibles) */
      .dir-lista { overflow: hidden; }
      .dir-row { border-bottom: 1px solid var(--line); }
      .dir-row:last-child { border-bottom: 0; }
      .dir-row__head { width: 100%; display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: transparent; border: 0; text-align: left; }
      .dir-row__head:hover { background: var(--sand); }
      .dir-row__info { flex: 1; display: flex; flex-direction: column; gap: 1px; min-width: 0; }
      .dir-row__info strong { display: flex; align-items: center; gap: 8px; }
      .gen { font-size: 10px; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; color: var(--ink-2); background: var(--sand); border-radius: 99px; padding: 2px 8px; }
      .qbadge { width: 26px; height: 26px; border-radius: 7px; display: grid; place-items: center; font-weight: 800; font-size: 12px; flex-shrink: 0; }
      ...
      .dir-row__chev { color: var(--ink-3); transition: transform 0.2s ease; flex-shrink: 0; }
      .dir-row--open .dir-row__chev { transform: rotate(90deg); }

      /* Perfil expandible: 4 dimensiones 2×2 */
      .dir-profile { padding: 4px 16px 18px 50px; }
      .dim-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
      .dim { background: var(--sand); border-radius: var(--radius-s); padding: 12px 14px; display: flex; flex-direction: column; gap: 8px; }
      .dim__h { display: flex; align-items: center; gap: 6px; font-size: 11px; font-weight: 800; letter-spacing: 0.04em; text-transform: uppercase; color: var(--ink-2); }
      .dim__row { display: flex; align-items: center; gap: 10px; }
      .empr-n strong { font-family: var(--font-display); font-size: 17px; }
      .par-est { color: var(--brand-600); font-size: 15px; letter-spacing: 1px; }
      .mp { display: grid; grid-template-columns: 96px 1fr auto; align-items: center; gap: 10px; }
      .mp__l { font-size: 12px; color: var(--ink-2); }
      .mp__v { font-size: 11.5px; white-space: nowrap; }
      .dim__act { display: flex; gap: 10px; margin-top: 12px; flex-wrap: wrap; }

      /* Sanas (Cuadrante A) compactas */
      .sanas { padding: 14px 16px; background: var(--sand); }
      .sanas__chips { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px; }
      .sana-chip { display: inline-flex; align-items: center; gap: 7px; border-radius: 99px; border: 1px solid var(--success); background: var(--success-bg); color: var(--success); font-size: 12.5px; font-weight: 700; padding: 6px 12px; }
      .sana-chip .gen { background: transparent; color: var(--success); padding: 0; }

      .dir-foot { margin: 14px 2px 0; line-height: 1.6; } .dir-foot a { color: var(--brand-600); font-weight: 700; }
      ...
      @media (max-width: 720px) {
        ...
        .gp-ind { grid-template-columns: 1fr; }
...
export class Equipo {
  ...
  protected readonly lente = signal<'prioridad' | 'cuadrante' | 'metas' | 'empr' | 'par'>('prioridad');
  ...
  protected readonly dist = (() => {
    ...
    return { A, B, C, D, partes: [{ k: 'A', n: A }, { k: 'B', n: B }, { k: 'C', n: C }, { k: 'D', n: D }] };
  })();
  ...
  protected lenteHint(): string {
    return {
      prioridad: 'Primero quién necesita ayuda (D → C → B → A); las sanas en A, compactas al final.',
  ...
  private readonly ordenadas = computed<Directora[]>(() => {
    ...
    switch (this.lente()) {
      case 'prioridad':
        arr.sort((a, b) => rank[a.cuadrante] - rank[b.cuadrante] || (this.sinEmpr(a) ? 0 : 1) - (this.sinEmpr(b) ? 0 : 1));
        break;
      case 'cuadrante':
      ...
      case 'metas':
        ...
        break;
      case 'empr':
        arr.sort((a, b) => (this.sinEmpr(b) ? 1 : 0) - (this.sinEmpr(a) ? 1 : 0) ||
  ...
  protected readonly principales = computed<Directora[]>(() =>
    this.lente() === 'prioridad' ? this.ordenadas().filter((d) => d.cuadrante !== 'A') : this.ordenadas(),
  );
  ...
  protected cumpl(d: Directora): number {
    ...
    const avg = parts.reduce((s, p) => s + Math.min(1, p.meta ? p.real / p.meta : 0), 0) / parts.length;
  ...
  protected sub(d: Directora): string {
    switch (this.lente()) {
      ...
      case 'par':
        return d.par.proyectada ? `Proyecta Estrella ${d.par.proyectada} · Sueño ${d.par.sueno}` : d.par.falta;
      default:
