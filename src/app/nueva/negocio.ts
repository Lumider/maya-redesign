      ...
      <div class="v2-grid">
          ...
          <section class="card pad v2-section" appReveal>
            ...
            <div class="cuad-prog">
              ...
              <div class="cuad-prog__pped">
                <div class="tiny bad" style="margin-bottom:6px;font-weight:700">Primeros pedidos · {{ c.primerosPedidos.valor }} de {{ c.cuadrante.ppedRequeridos }} · te mantiene en Cuadrante D</div>
          ...
          <section id="resultados" class="v2-section" appReveal>
            ...
            <div class="ind-grid">
              @for (r of resultados; track r.label) {
                <div class="tile card">
                  <div class="tile__top"><span class="tile__label">{{ r.label }}</span><span class="sem" [class]="'sem--' + r.tone"></span></div>
          ...
          <section class="v2-section" appReveal>
            ...
            <div class="card pad">
              <div class="row-between" style="margin-bottom:6px">
                <span class="tiny" style="font-weight:700">{{ enA }} de {{ completadas }} campañas en Cuadrante A</span>
                <span class="badge badge--brand">Excelencia GP · {{ rec.excelenciaGP.medalla }}</span>
              </div>
              <div class="hist__row">
              ...
              <span class="tiny">Medalla por {{ rec.excelenciaGP.cumplidas }} de {{ rec.excelenciaGP.de }} campañas en Cuadrante A.</span>
            </div>
          </section>
        </main>
        ...
        <aside class="v2-aside">
          <div id="deuda" class="card pad" appReveal>
            ...
            <div class="alert alert--warning" style="margin:10px 0"><app-icon name="alert" [size]="16" /> Tu grupo necesita pagar \${{ c.morosidad.pagoNecesario | number }} para un IM saludable.</div>
            <div class="acc">
              <a class="acc__link" routerLink="/n/equipo"><app-icon name="wallet" [size]="15" /> Créditos por aprobar →</a>
          ...
          <div class="card pad" appReveal [revealDelay]="80">
            ...
            <div class="row-between"><span class="badge badge--success">{{ c.credito.estado }}</span></div>
            <div style="display:flex;gap:16px;align-items:center;margin-top:10px">
              <app-ring [pct]="credPct" [size]="92" label="usado" [expected]="0" />
              <div>
                <div class="kv"><span>Disponible</span><strong>\${{ c.credito.disponible | number }}</strong></div>
  ...
  styles: [
    `
      ...
      .hero-estado__q {
        ...
        font-size: 46px; font-weight: 800; color: var(--danger);
        background: var(--danger-bg); border: 2px solid var(--danger);
      }
      .hero-estado__body { display: flex; flex-direction: column; gap: 4px; }
      .hero-estado__t { font-size: 18px; margin: 2px 0; }
      .hero-estado__body p { margin: 0; font-size: 13.5px; }

      .matrix { position: relative; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding: 0 0 26px 26px; margin-top: 4px; }
      ...
      .cuad-prog { display: flex; gap: 22px; align-items: center; margin-top: 16px; flex-wrap: wrap; }
      .cuad-prog__ring { text-align: center; }
      .cuad-prog__pped { flex: 1; min-width: 220px; }
      /* PPED en 0/4 = crítico (rojo), no ámbar */
      .dots--bad .dot { border-color: var(--danger); color: var(--danger); }

      .hist__row { display: flex; gap: 6px; overflow-x: auto; padding: 8px 2px; }
      ...
      @media (max-width: 720px) {
        ...
        .hero-estado__q { width: 64px; height: 64px; font-size: 34px; }
      }
    `,
  ],
})
export class Negocio {
  ...
  protected readonly resultados = [
    { label: 'Venta GP', valor: '$' + CAMPANA.ventaActual.toLocaleString('en-US'), detalle: `${this.mrmPct}% del MRM · ${this.metaPct}% de meta C6`, tone: 'warn' },
    { label: 'Activas GP', valor: `${CAMPANA.activas.total} / ${CAMPANA.activas.meta}`, detalle: 'meta campañal de activas', tone: 'warn' },
    { label: 'Primeros pedidos', valor: `${CAMPANA.primerosPedidos.valor} / ${CAMPANA.cuadrante.ppedRequeridos}`, detalle: 'requisito de Cuadrante A', tone: 'bad' },
  ];
}
